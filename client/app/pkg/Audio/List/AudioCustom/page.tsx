        ...
        <div className="max-w-screen-4xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8 lg:py-16">
          <div className="max-w-screen-2xl">
            <h2 className="text-3xl font-bold sm:text-4xl text-red-600">
              Viewing YtDlx.AudioOnly.List.Custom()
            </h2>
            ...
            <ul className="list-disc m-4 bg-neutral-800/40 shadow-black shadow-2xl p-8 rounded-3xl border border-dashed border-red-600">
              <li>
                Downloads and processes a List YouTube video with audio
...
(async () => {
  try {
    ...
    for (const resolution of resolutions) {
      ...
      await ytdlx.AudioOnly.List.Custom({
        resolution,
        stream: false, 
        verbose: true, 
        onionTor: false, 
      ...
      const result = await ytdlx.AudioOnly.List.Custom({
        resolution,
        stream: false, 
        verbose: true, 
        onionTor: false, 
...
(async () => {
  try {
    ...
    for (const resolution of resolutions) {
      console.log(colors.blue("@test:"), "Download Custom audio");
      await ytdlx.default.AudioOnly.List.Custom({
        resolution,
        stream: false, 
        verbose: true, 
        onionTor: false, 
      ...
      console.log(colors.blue("@test:"), "(stream) Download Custom audio");
      const result = await ytdlx.default.AudioOnly.List.Custom({
        resolution,
        stream: false, 
        verbose: true, 
        onionTor: false, 
