...
async function closers(browser2) {
  try {
    ...
    console.error(colors28__default.default.red("@error:"), error);
  }
}
...
async function Engine(query) {
  try {
    ...
    if (proLoc !== "") {
      ...
      throw new Error(
        colors28__default.default.red("@error: ") + "could not find the engine file."
      );
    }
...
async function lowEntry(metaBody) {
  if (!metaBody || metaBody.length === 0) {
    console.log(colors28__default.default.red("@error:"), "sorry no downloadable data found");
    return void 0;
  }
...
async function AudioVideoQualityCustom(input) {
  try {
    ...
    if (engineData === void 0) {
      ...
      if (AudioData === void 0) {
        throw new Error(
          colors28__default.default.red("@error: ") + AQuality + " not found in the video."
        );
      } else if (VideoData === void 0) {
        throw new Error(
          colors28__default.default.red("@error: ") + VQuality + " not found in the video."
        );
      } else {
        const ffmpeg = ffmpeg_default({
          input: VideoData.AVDownload.mediaurl,
