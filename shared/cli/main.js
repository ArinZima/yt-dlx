...
async function Engine(query) {
  try {
    ...
    while (maxTries2 > 0) {
      const enginePath = path2__namespace.join(currentDir2, "util", "engine");
      if (fs__namespace.existsSync(enginePath)) {
        proLoc = enginePath;
        break;
      } else {
        currentDir2 = path2__namespace.join(currentDir2, "..");
        maxTries2--;
      }
    }
    if (proLoc !== "") {
...
var color = colors24__default.default.green;
...
function gpuffmpeg(input, verbose) {
  const ffmpeg = fluent__default.default().input(input).on("start", (command) => {
    if (verbose)
      console.log(colors24__default.default.green("\n@ffmpeg:"), command);
  }).on("progress", ({ percent, timemark }) => {
    progressBar_default({ timemark, percent });
  }).on("end", () => console.log(colors24__default.default.green("\n@ffmpeg:"), "ended")).on("close", () => console.log(colors24__default.default.green("\n@ffmpeg:"), "closed")).on("finish", () => console.log(colors24__default.default.green("\n@ffmpeg:"), "finish")).on("error", (e) => console.error(colors24__default.default.red("\n@ffmpeg:"), e.message));
  while (maxTries > 0) {
  ...
  switch (true) {
    case (vendor && vendor.includes("NVIDIA")):
      console.log(colors24__default.default.green("@ffmpeg: using GPU " + vendor));
