...
async function runFunc(runLogic) {
  ...
  while (retry4 < 6) {
    try {
      ...
      retry4++;
      console.log(colors29__default.default.yellow("@retry:"), `${retry4}/6.`);
      if (retry4 >= 6)
        throw new Error(colors29__default.default.red("@error:"), error.message);
...
async function gpuffmpeg({
  ...
  const response = await runFunc(async () => {
    ...
    const ffmpeg = fluent__default.default(input).on("start", (command) => {
      if (verbose)
        console.log(colors29__default.default.green("@ffmpeg:"), command);
    }).on("progress", (prog) => progressBar(prog, size)).on("end", () => console.log("\n")).on("error", (e) => console.error(colors29__default.default.red("\n@ffmpeg:"), e.message));
    while (maxTries > 0) {
    ...
    switch (true) {
      case (vendor && vendor.includes("NVIDIA")):
        console.log(
          ...
          "using GPU",
          colors29__default.default.green(vendor)
        );
      ...
      default:
        console.log(
          colors29__default.default.yellow("@ffmpeg:"),
