...
var path20__namespace = /*#__PURE__*/_interopNamespace(path20);
...
async function SearchVideos(input) {
  ...
  let playlistMeta = [];
  let TubeResp;
  console.log(colors29__default.default.green("@scrape:"), "booting chromium...");
  switch (input.type) {
    case "video":
      ...
      if (screenshot) {
        ...
        console.log(colors29__default.default.yellow("@scrape:"), "took snapshot...");
      }
      ...
      console.log(
        ...
        colors29__default.default.white("scrapping done for"),
        query
      );
      TubeResp = metaTube;
      break;
    ...
    default:
      console.log(
        ...
        colors29__default.default.white("wrong filter type provided.")
      );
      TubeResp = void 0;
      break;
  }
...
async function Engine({
  ...
  while (maxT > 0) {
    const enginePath = path20__namespace.join(dirC, "util", "engine");
    if (fs2__namespace.existsSync(enginePath)) {
      pLoc = enginePath;
      break;
    } else {
      dirC = path20__namespace.join(dirC, "..");
      maxT--;
    }
  }
...
async function Agent({
  ...
  console.log(
    colors29__default.default.green("@info:"),
    "system",
    colors29__default.default.green("ipAddress"),
    nipTor.stdout.trim()
  );
...
async function proTube({
  ...
  console.log(
    colors29__default.default.green("@ffmpeg:"),
    "using",
    colors29__default.default.green("ipAddress"),
    ipAddress
  );
  const numCores = os__namespace.cpus().length;
...
async function AudioLowest(input) {
  ...
  if (engineData === void 0) {
    ...
    const folder = output ? path20__namespace.join(process.cwd(), output) : process.cwd();
    ...
    if (stream) {
      return {
        ffmpeg: ffmpeg2,
        filename: output ? path20__namespace.join(folder, filename) : filename.replace("_)_", ")_")
      };
    } else {
      await new Promise((resolve, _reject) => {
        ffmpeg2.output(path20__namespace.join(folder, filename.replace("_)_", ")_")));
...
var program = async () => {
  ...
  switch (command) {
    case "install:deps":
      while (maxTries > 0) {
        const enginePath = path20__namespace.join(currentDir, "util");
        if (fs2__namespace.existsSync(enginePath)) {
          uLoc = enginePath;
          break;
        } else {
          currentDir = path20__namespace.join(currentDir, "..");
          maxTries--;
        }
      }
      ...
      await Promise.all([
        new Promise((resolve, reject2) => {
          rox.stdout.on("data", (stdout) => {
            console.log(colors29__default.default.green("@stdout:"), stdout.toString().trim());
          });
          rox.on("close", (code) => {
            if (code === 0)
              resolve();
        ...
        new Promise((resolve, reject2) => {
          rox.stderr.on("data", (stderr) => {
            console.log(colors29__default.default.yellow("@stderr:"), stderr.toString().trim());
          });
          rox.on("close", (code) => {
            if (code === 0)
              resolve();
