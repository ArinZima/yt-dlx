...
var colors23__default = /*#__PURE__*/_interopDefault(colors23);
var puppeteer__default = /*#__PURE__*/_interopDefault(puppeteer);
var fs2__namespace = /*#__PURE__*/_interopNamespace(fs2);
var path14__namespace = /*#__PURE__*/_interopNamespace(path14);
...
async function SearchVideos(input) {
  const QuerySchema = z4.z.object({
    ...
    autoSocks5: z4.z.boolean().optional(),
  ...
  let playlistMeta = [];
  let TubeResp;
  console.log(colors23__default.default.green("@scrape:"), "booting chromium...");
  switch (input.type) {
    case "video":
      ...
      if (screenshot) {
        ...
        console.log(colors23__default.default.yellow("@scrape:"), "took snapshot...");
      }
      ...
      console.log(
        ...
        colors23__default.default.white("scrapping done for"),
        query
      );
      TubeResp = metaTube;
      break;
    ...
    default:
      console.log(
        colors23__default.default.red("@error:"),
        colors23__default.default.white("wrong filter type provided.")
      );
      TubeResp = void 0;
      break;
  }
...
function help() {
  ...
  return Promise.resolve(
    colors23__default.default.bold.white(`
\u2715\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2715
...
async function Engine({
  ...
  let LowVideoHDR = {};
  let HighVideoHDR = {};
  let LowAudioFileSizeDataDRC = {};
  let HighAudioFileSizeDataDRC = {};
  let LowAudioFileSizeData = {};
  let HighAudioFileSizeData = {};
  let LowVideoFileSizeDataHDR = {};
  let HighVideoFileSizeDataHDR = {};
  let LowVideoFileSizeData = {};
  let HighVideoFileSizeData = {};
  let maxT = 8, pLoc = "", dirC = process.cwd();
  while (maxT > 0) {
    const enginePath = path14__namespace.join(dirC, "util", "engine");
    switch (true) {
      case fs2__namespace.existsSync(enginePath):
        pLoc = enginePath;
        maxT = 0;
        break;
      default:
        dirC = path14__namespace.join(dirC, "..");
        maxT--;
        break;
    }
  }
  ...
  i.formats.forEach((tube) => {
    ...
    switch (true) {
      case (!rm.has(tube.format_note) && tube.protocol === "m3u8_native" && tube.vbr):
        switch (true) {
          case (!LowManifest[tube.resolution] || tube.vbr < LowManifest[tube.resolution].vbr):
            LowManifest[tube.resolution] = tube;
            break;
          case (!HighManifest[tube.resolution] || tube.vbr > HighManifest[tube.resolution].vbr):
            HighManifest[tube.resolution] = tube;
            break;
        }
        break;
      case (rm.has(tube.format_note) || tube.filesize === void 0 || tube.filesize === null):
        return;
      default:
        switch (true) {
          ...
          case tube.format_note.includes("HDR"):
            switch (true) {
              case (!LowVideoHDR[tube.format_note] || tube.filesize < LowVideoHDR[tube.format_note].filesize):
                LowVideoHDR[tube.format_note] = tube;
                break;
              case (!HighVideoHDR[tube.format_note] || tube.filesize > HighVideoHDR[tube.format_note].filesize):
    ...
    switch (true) {
      case tube.format_note.includes("DRC"):
        switch (true) {
          case tube.filesize:
            switch (true) {
              case (!LowAudioFileSizeDataDRC[tube.format_note] || tube.filesize < LowAudioFileSizeDataDRC[tube.format_note].filesize):
                LowAudioFileSizeDataDRC[tube.format_note] = tube;
                break;
              case (!HighAudioFileSizeDataDRC[tube.format_note] || tube.filesize > HighAudioFileSizeDataDRC[tube.format_note].filesize):
      ...
      case tube.format_note.includes("HDR"):
        switch (true) {
          case tube.filesize:
            switch (true) {
              case (!LowVideoFileSizeDataHDR[tube.format_note] || tube.filesize < LowVideoFileSizeDataHDR[tube.format_note].filesize):
                LowVideoFileSizeDataHDR[tube.format_note] = tube;
                break;
              case (!HighVideoFileSizeDataHDR[tube.format_note] || tube.filesize > HighVideoFileSizeDataHDR[tube.format_note].filesize):
          ...
          default:
            switch (true) {
              case (!LowVideo[tube.format_note] || tube.filesize < LowVideo[tube.format_note].filesize):
                LowVideo[tube.format_note] = tube;
                break;
              case (!HighVideo[tube.format_note] || tube.filesize > HighVideo[tube.format_note].filesize):
                HighVideo[tube.format_note] = tube;
                break;
            }
            break;
      ...
      default:
        switch (true) {
          case tube.filesize:
            switch (true) {
              case (!LowAudioFileSizeData[tube.format_note] || tube.filesize < LowAudioFileSizeData[tube.format_note].filesize):
                LowAudioFileSizeData[tube.format_note] = tube;
                break;
              case (!HighAudioFileSizeData[tube.format_note] || tube.filesize > HighAudioFileSizeData[tube.format_note].filesize):
                HighAudioFileSizeData[tube.format_note] = tube;
                break;
            }
            switch (true) {
              case (!LowVideoFileSizeData[tube.format_note] || tube.filesize < LowVideoFileSizeData[tube.format_note].filesize):
                LowVideoFileSizeData[tube.format_note] = tube;
                break;
              case (!HighVideoFileSizeData[tube.format_note] || tube.filesize > HighVideoFileSizeData[tube.format_note].filesize):
  ...
  function mapVideo(videoObject) {
    return propfilter(Object.values(videoObject)).map((i2) => ({
      filesize: i2.filesize,
      filesizeP: sizeFormat(i2.filesize),
      format_id: parseFloat(i2.format_id),
      format_note: i2.format_note,
      fps: parseFloat(i2.fps),
      height: parseFloat(i2.height),
      width: parseFloat(i2.width),
      has_drm: i2.has_drm,
      tbr: parseFloat(i2.tbr),
      url: i2.url,
      ext: i2.ext,
      vcodec: i2.vcodec,
      dynamic_range: i2.dynamic_range,
      container: i2.container,
      resolution: i2.resolution,
      aspect_ratio: parseFloat(i2.aspect_ratio),
      video_ext: i2.video_ext,
      vbr: parseFloat(i2.vbr),
      format: i2.format
    }));
  }
  function mapManifest(manifestObject) {
    return Object.values(manifestObject).map((i2) => ({
      format_id: parseFloat(i2.format_id),
      url: i2.url,
      manifest_url: i2.manifest_url,
      tbr: parseFloat(i2.tbr),
      ext: i2.ext,
      fps: parseFloat(i2.fps),
      quality: parseFloat(i2.quality),
      has_drm: i2.has_drm,
      width: parseFloat(i2.width),
      height: parseFloat(i2.height),
      vcodec: i2.vcodec,
      ...
      aspect_ratio: parseFloat(i2.aspect_ratio),
      video_ext: i2.video_ext,
      audio_ext: i2.audio_ext,
      abr: parseFloat(i2.abr),
      vbr: parseFloat(i2.vbr),
      format: i2.format
    }));
  }
  function mapAudio(audioObject) {
    return propfilter(Object.values(audioObject)).map((i2) => ({
      filesize: i2.filesize,
      filesizeP: sizeFormat(i2.filesize),
      asr: parseFloat(i2.asr),
      format_id: i2.format_id,
      format_note: i2.format_note,
      has_drm: i2.has_drm,
      tbr: parseFloat(i2.tbr),
      url: i2.url,
...
async function Agent({
  ...
  console.log(
    ...
    colors23__default.default.green("yt-dlx"),
    "version",
    colors23__default.default.green(version)
  );
  let nipTor;
  ...
  console.log(
    colors23__default.default.green("@info:"),
    "system",
    colors23__default.default.green("ipAddress"),
    nipTor.stdout.trim()
  );
  ...
  if (!videoId) {
    ...
    if (!TubeBody[0]) {
      ...
      console.log(
        ...
        colors23__default.default.green(TubeBody[0].title)
      );
    ...
    if (!TubeBody) {
      ...
      console.log(
        colors23__default.default.green("@info:"),
        `preparing payload for`,
        colors23__default.default.green(TubeBody.title)
      );
      respEngine = await Engine({
...
async function extract_playlist_videos({
  ...
  await eachSeries$1(playlistUrls, async (listLink) => {
    ...
    if (query === void 0) {
      console.error(
        colors23__default.default.bold.red("@error: "),
      ...
      if (resp === void 0) {
        ...
        console.log(
          ...
          colors23__default.default.green(resp.playlistTitle),
...
function progressBar(prog) {
  ...
  let color = colors23__default.default.green;
  if (prog.percent > 98)
    prog.percent = 100;
  if (prog.percent < 25)
    color = colors23__default.default.red;
  else if (prog.percent < 50)
    color = colors23__default.default.yellow;
...
async function AudioLowest(input) {
  ...
  if (engineData === void 0) {
    ...
    const folder = output ? path14__namespace.join(process.cwd(), output) : process.cwd();
    if (!fs2__namespace.existsSync(folder))
      fs2__namespace.mkdirSync(folder, { recursive: true });
    let filename = "yt-dlx_(AudioLowest_";
    const numThreads = os__namespace.cpus().length * 2;
    const ff = ffmpeg__default.default();
    const adata = Array.isArray(engineData.LowAudioDRC) && engineData.LowAudioDRC.length > 0 ? engineData.LowAudioDRC[0]?.url : engineData.LowAudio[0]?.url ?? void 0;
    ...
    if (adata) {
      ...
      throw new Error(colors23__default.default.red("@error: ") + "No audio data found.");
    ...
    if (stream) {
      return {
        ffmpeg: ff,
        filename: output ? path14__namespace.join(folder, filename) : filename.replace("_)_", ")_")
      };
    } else {
      await new Promise((resolve, reject2) => {
        ff.output(path14__namespace.join(folder, filename.replace("_)_", ")_")));
        ff.on("end", () => resolve());
        ff.on("error", (error) => {
          reject2(new Error(colors23__default.default.red("@error: ") + error.message));
        });
        ff.run();
      });
    }
...
async function AudioHighest(input) {
  ...
  if (engineData === void 0) {
    ...
    const numThreads = os__namespace.cpus().length * 2;
    const ff = ffmpeg__default.default();
    const adata = Array.isArray(engineData.HighAudioDRC) && engineData.HighAudioDRC.length > 0 ? engineData.HighAudioDRC[engineData.HighAudioDRC.length - 1]?.url : engineData.HighAudio[engineData.HighAudio.length - 1]?.url ?? void 0;
    ff.addOption("-threads", numThreads.toString());
    ff.addOption("-headers", "X-Forwarded-For: " + engineData.ipAddress);
    if (adata) {
      console.log(adata);
...
// core/pipes/audio/list/ListAudioLowest.ts
var qconf3 = z4.z.object({
  output: z4.z.string().optional(),
  verbose: z4.z.boolean().optional(),
  autoSocks5: z4.z.boolean().optional(),
  query: z4.z.array(
    z4.z.string().min(1).refine(
      async (input) => {
        switch (true) {
          case /^(https?:\/\/)?(www\.)?(youtube\.com\/(playlist\?|embed\/|v\/|channel\/)(list=)?)([a-zA-Z0-9_-]+)/.test(
            input
...
async function ListAudioLowest(input) {
  const { query, output, verbose, filter: filter2, autoSocks5 } = await qconf3.parseAsync(
    input
  );
  ...
  for (const pURL of query) {
    try {
      ...
      console.log(colors23__default.default.red("@error:"), error);
      continue;
    }
  }
  console.log(
    ...
    colors23__default.default.yellow(vDATA.size.toString())
  );
  for (const video of vDATA) {
    try {
      ...
      if (!fs2__namespace.existsSync(folder))
        fs2__namespace.mkdirSync(folder, { recursive: true });
      let filename = "yt-dlx_(AudioLowest_";
      const ffmpeg6 = await proTube({
        adata: engineData.LowAudioDRC || engineData.LowAudio,
        ipAddress: engineData.ipAddress
      });
      ffmpeg6.withOutputFormat("avi");
      switch (filter2) {
        case "bassboost":
          ffmpeg6.withAudioFilter(["bass=g=10,dynaudnorm=f=150"]);
          filename += `bassboost)_${title}.avi`;
          break;
        case "echo":
          ffmpeg6.withAudioFilter(["aecho=0.8:0.9:1000:0.3"]);
          filename += `echo)_${title}.avi`;
          break;
        case "flanger":
          ffmpeg6.withAudioFilter(["flanger"]);
          filename += `flanger)_${title}.avi`;
          break;
        case "nightcore":
          ffmpeg6.withAudioFilter(["aresample=48000,asetrate=48000*1.25"]);
          filename += `nightcore)_${title}.avi`;
          break;
        case "panning":
          ffmpeg6.withAudioFilter(["apulsator=hz=0.08"]);
          filename += `panning)_${title}.avi`;
          break;
        case "phaser":
          ffmpeg6.withAudioFilter(["aphaser=in_gain=0.4"]);
          filename += `phaser)_${title}.avi`;
          break;
        case "reverse":
          ffmpeg6.withAudioFilter(["areverse"]);
          filename += `reverse)_${title}.avi`;
          break;
        case "slow":
          ffmpeg6.withAudioFilter(["atempo=0.8"]);
          filename += `slow)_${title}.avi`;
          break;
        case "speed":
          ffmpeg6.withAudioFilter(["atempo=2"]);
          filename += `speed)_${title}.avi`;
          break;
        case "subboost":
          ffmpeg6.withAudioFilter(["asubboost"]);
          filename += `subboost)_${title}.avi`;
          break;
        case "superslow":
          ffmpeg6.withAudioFilter(["atempo=0.5"]);
          filename += `superslow)_${title}.avi`;
          break;
        case "superspeed":
          ffmpeg6.withAudioFilter(["atempo=3"]);
          filename += `superspeed)_${title}.avi`;
          break;
        case "surround":
          ffmpeg6.withAudioFilter(["surround"]);
          filename += `surround)_${title}.avi`;
          break;
        case "vaporwave":
          ffmpeg6.withAudioFilter(["aresample=48000,asetrate=48000*0.8"]);
          filename += `vaporwave)_${title}.avi`;
          break;
        case "vibrato":
          ffmpeg6.withAudioFilter(["vibrato=f=6.5"]);
          filename += `vibrato)_${title}.avi`;
          break;
        default:
          filename += `)_${title}.avi`;
      ...
      await new Promise((resolve, _reject) => {
        ffmpeg6.output(path14__namespace.join(folder, filename.replace("_)_", ")_")));
        ffmpeg6.on("end", () => resolve());
        ffmpeg6.on("error", (error) => {
          throw new Error(colors23__default.default.red("@error: ") + error.message);
        });
        ffmpeg6.run();
      });
    } catch (error) {
...
async function VideoLowest(input) {
  const { query, stream, verbose, output, filter: filter2, autoSocks5 } = await qconf5.parseAsync(input);
  const engineData = await Agent({ query, verbose, autoSocks5 });
  if (engineData === void 0) {
    throw new Error(
      colors23__default.default.red("@error: ") + "unable to get response from youtube."
    ...
    const numThreads = os__namespace.cpus().length * 2;
    const ff = ffmpeg__default.default();
    const vdata = Array.isArray(engineData.LowManifest) && engineData.LowManifest.length > 0 ? engineData.LowManifest[0]?.url : void 0;
    ff.outputOptions("-c copy");
    ff.addOption("-threads", numThreads.toString());
    if (vdata) {
      console.log(vdata);
      ...
      throw new Error(colors23__default.default.red("@error: ") + "No video data found.");
...
async function VideoHighest(input) {
  const { query, stream, verbose, output, filter: filter2, autoSocks5 } = await qconf6.parseAsync(input);
  const engineData = await Agent({ query, verbose, autoSocks5 });
  if (engineData === void 0) {
    throw new Error(
      colors23__default.default.red("@error: ") + "unable to get response from youtube."
    ...
    const numThreads = os__namespace.cpus().length * 2;
    const ff = ffmpeg__default.default();
    const vdata = Array.isArray(engineData.HighManifest) && engineData.HighManifest.length > 0 ? engineData.HighManifest[engineData.HighManifest.length - 1]?.url : void 0;
    ff.outputOptions("-c copy");
    ff.addOption("-threads", numThreads.toString());
    if (vdata) {
      console.log(vdata);
...
// core/pipes/video/list/ListVideoLowest.ts
...
async function ListVideoLowest(input) {
  const { query, output, verbose, filter: filter2, autoSocks5 } = await qconf7.parseAsync(
    input
  );
  const vDATA = /* @__PURE__ */ new Set();
  for (const pURL of query) {
  ...
  for (const video of vDATA) {
    try {
      ...
      ffmpeg6.withOutputFormat("matroska");
      switch (filter2) {
        case "grayscale":
          ffmpeg6.withVideoFilter(
            "colorchannelmixer=.3:.4:.3:0:.3:.4:.3:0:.3:.4:.3"
          );
          filename += `grayscale)_${title}.mkv`;
          break;
        case "invert":
          ffmpeg6.withVideoFilter("negate");
          filename += `invert)_${title}.mkv`;
          break;
        case "rotate90":
          ffmpeg6.withVideoFilter("rotate=PI/2");
          filename += `rotate90)_${title}.mkv`;
          break;
        case "rotate180":
          ffmpeg6.withVideoFilter("rotate=PI");
          filename += `rotate180)_${title}.mkv`;
          break;
        case "rotate270":
          ffmpeg6.withVideoFilter("rotate=3*PI/2");
          filename += `rotate270)_${title}.mkv`;
          break;
        case "flipHorizontal":
          ffmpeg6.withVideoFilter("hflip");
          filename += `flipHorizontal)_${title}.mkv`;
          break;
        case "flipVertical":
          ffmpeg6.withVideoFilter("vflip");
          filename += `flipVertical)_${title}.mkv`;
          break;
        default:
          filename += `)_${title}.mkv`;
...
// core/pipes/video/list/ListVideoHighest.ts
...
async function ListVideoHighest(input) {
  const { query, verbose, output, filter: filter2, autoSocks5 } = await qconf8.parseAsync(
    input
  );
  const vDATA = /* @__PURE__ */ new Set();
  for (const pURL of query) {
...
async function AudioVideoLowest(input) {
  const { query, stream, verbose, output, filter: filter2, autoSocks5 } = await qconf9.parseAsync(input);
  const engineData = await Agent({ query, verbose, autoSocks5 });
  if (engineData === void 0) {
    throw new Error(
      colors23__default.default.red("@error: ") + "unable to get response from youtube."
    ...
    ffmpeg6.addInput(AudioData.AVDownload.mediaurl);
    ffmpeg6.withOutputFormat("matroska");
...
async function AudioVideoHighest(input) {
  const { query, stream, verbose, output, filter: filter2, autoSocks5 } = await qconf10.parseAsync(input);
  const engineData = await Agent({ query, verbose, autoSocks5 });
  if (engineData === void 0) {
    throw new Error(
      colors23__default.default.red("@error: ") + "unable to get response from youtube."
...
async function ListAudioVideoLowest(input) {
  const { query, verbose, output, filter: filter2, autoSocks5 } = await qconf12.parseAsync(
    input
  );
  const vDATA = /* @__PURE__ */ new Set();
  for (const pURL of query) {
...
var proTube2 = minimist__default.default(process.argv.slice(2), {
  string: ["query", "format"],
  alias: {
    h: "help",
    e: "extract",
...
var program = async () => {
  ...
  switch (command) {
    case "install:deps":
      while (maxTries > 0) {
        const enginePath = path14__namespace.join(currentDir, "util");
        if (fs2__namespace.existsSync(enginePath)) {
          uLoc = enginePath;
          break;
        } else {
          currentDir = path14__namespace.join(currentDir, "..");
          maxTries--;
        }
      }
      ...
      await Promise.all([
        new Promise((resolve, reject2) => {
          rox.stdout.on("data", (stdout) => {
            console.log(colors23__default.default.green("@stdout:"), stdout.toString().trim());
          });
          rox.on("close", (code) => {
            if (code === 0)
              resolve();
        ...
        new Promise((resolve, reject2) => {
          rox.stderr.on("data", (stderr) => {
            console.log(colors23__default.default.yellow("@stderr:"), stderr.toString().trim());
          });
          rox.on("close", (code) => {
            if (code === 0)
              resolve();
    ...
    case "version":
    case "v":
      console.error(colors23__default.default.green("Installed Version: yt-dlx@" + version));
      break;
    case "help":
    case "h":
      core_default().info().help().then((data) => {
        ...
        console.error(colors23__default.default.red(error));
        process.exit();
      });
      break;
    case "extract":
    case "e":
      if (!proTube2 || !proTube2.query || proTube2.query.length === 0) {
        console.error(colors23__default.default.red("error: no query"));
      } else
    ...
    case "aqc":
      ...
      if (!proTube2 || !proTube2.format || proTube2.format.length === 0) {
        console.error(colors23__default.default.red("error: no format"));
      }
