...
async function Engine({
  ...
  while (maxT > 0) {
    const enginePath = path8__namespace.join(dirC, "util", "engine");
    if (fs2__namespace.existsSync(enginePath)) {
      pLoc = enginePath;
      break;
    } else {
      dirC = path8__namespace.join(dirC, "..");
      maxT--;
    }
  }
...
async function AudioLowest(input) {
  ...
  if (engineData === void 0) {
    ...
    const folder = output ? path8__namespace.join(process.cwd(), output) : process.cwd();
    ...
    if (stream) {
      return {
        ffmpeg: ff,
        filename: output ? path8__namespace.join(folder, filename) : filename.replace("_)_", ")_")
      };
    } else {
      await new Promise((resolve, reject2) => {
        ff.output(path8__namespace.join(folder, filename.replace("_)_", ")_")));
        ff.on("end", () => resolve());
        ff.on("error", (error) => {
...
var program = async () => {
  ...
  switch (command) {
    case "install:deps":
      while (maxTries > 0) {
        const enginePath = path8__namespace.join(currentDir, "util");
        if (fs2__namespace.existsSync(enginePath)) {
          uLoc = enginePath;
          break;
        } else {
          currentDir = path8__namespace.join(currentDir, "..");
          maxTries--;
        }
      }
