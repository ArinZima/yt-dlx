...
async function closers(browser2) {
  try {
    ...
    console.error(colors28.red("@error:"), error);
  }
}
...
async function Engine(query) {
  try {
    ...
    if (proLoc !== "") {
      ...
      throw new Error(
        colors28.red("@error: ") + "could not find the engine file."
      );
    }
...
function gpuffmpeg({
  ...
  switch (true) {
    case (vendor && vendor.includes("NVIDIA")):
      console.log(colors28.green("@ffmpeg:"), "using GPU", colors28.green(vendor));
      ffmpeg.withInputOption("-hwaccel cuda");
      ffmpeg.withVideoCodec("h264_nvenc");
      break;
    default:
      console.log(
        colors28.yellow("@ffmpeg:"),
...
async function lowEntry(metaBody) {
  if (!metaBody || metaBody.length === 0) {
    console.log(colors28.red("@error:"), "sorry no downloadable data found");
    return void 0;
  }
...
async function AudioQualityCustom(input) {
  try {
    const { query, stream, verbose, output, quality, filter: filter2 } = await qconf3.parseAsync(input);
    const engineData = await Agent({ query, verbose });
    if (engineData === void 0) {
      throw new Error(
        colors28.red("@error: ") + "unable to get response from youtube."
...
async function ListAudioLowest(input) {
  try {
    const { query, output, verbose, filter: filter2 } = await qconf4.parseAsync(input);
...
async function ListAudioHighest(input) {
  try {
    const { query, output, verbose, filter: filter2 } = await qconf5.parseAsync(input);
    const playlistData = await web_default.search.PlaylistInfo({ query });
    if (playlistData === void 0) {
      throw new Error(
        colors28.red("@error: ") + "unable to get response from youtube."
...
async function VideoHighest(input) {
  try {
    const { query, stream, verbose, output, filter: filter2 } = await qconf7.parseAsync(
      input
    );
    const engineData = await Agent({ query, verbose });
    if (engineData === void 0) {
...
async function ListVideoLowest(input) {
  try {
    const { query, output, verbose, filter: filter2 } = await qconf9.parseAsync(input);
    const playlistData = await web_default.search.PlaylistInfo({ query });
    if (playlistData === void 0) {
      throw new Error(
        colors28.red("@error: ") + "unable to get response from youtube."
...
async function ListVideoHighest(input) {
  try {
    const { query, verbose, output, filter: filter2 } = await qconf10.parseAsync(input);
    const playlistData = await web_default.search.PlaylistInfo({ query });
    if (playlistData === void 0) {
      throw new Error(
        colors28.red("@error: ") + "unable to get response from youtube."
...
async function AudioVideoLowest(input) {
  try {
    const { query, stream, verbose, output, filter: filter2 } = await qconf11.parseAsync(
      input
    );
    const engineData = await Agent({ query, verbose });
    if (engineData === void 0) {
...
async function AudioVideoHighest(input) {
  try {
    const { query, stream, verbose, output, filter: filter2 } = await qconf12.parseAsync(
      input
    );
    const engineData = await Agent({ query, verbose });
    if (engineData === void 0) {
...
async function AudioVideoQualityCustom(input) {
  try {
    const { query, stream, verbose, output, VQuality, AQuality, filter: filter2 } = await qconf13.parseAsync(input);
    const engineData = await Agent({ query, verbose });
    if (engineData === void 0) {
      throw new Error(
        colors28.red("@error: ") + "unable to get response from youtube."
      ...
      if (AudioData === void 0) {
        throw new Error(
          colors28.red("@error: ") + AQuality + " not found in the video."
        );
      } else if (VideoData === void 0) {
        throw new Error(
          colors28.red("@error: ") + VQuality + " not found in the video."
        );
      } else {
        const ffmpeg = ffmpeg_default({
          input: VideoData.AVDownload.mediaurl,
