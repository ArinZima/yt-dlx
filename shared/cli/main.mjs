...
async function Engine(query, port, proxy, username, password) {
  ...
  if (proLoc !== "") {
    ...
    throw new Error(colors29.red("@error: ") + "could not find the engine file.");
  }
...
async function Agent({
  ...
  let TubeBody;
  console.log(colors29.bold.green("@info: ") + `using yt-dlx version ${version}`);
  switch (true) {
    case (!query || query.trim() === ""):
      throw new Error(colors29.bold.red("@error: ") + "'query' is required.");
    case (/https/i.test(query) && /list/i.test(query)):
      throw new Error(
        colors29.bold.red("@error: ") + "use extract_playlist_videos()."
      );
    case (/https/i.test(query) && !/list/i.test(query)):
  ...
  console.log(colors29.bold.green("@info: ") + `fetching metadata for ${query}`);
  switch (videoId) {
    case null:
      ...
      if (!TubeBody || TubeBody.length === 0) {
        throw new Error(
          colors29.bold.red("@error: ") + "no data returned from server."
        );
      } else if (TubeBody[0]) {
        console.log(
          colors29.bold.green("@info: ") + `preparing payload for ${TubeBody[0].title}`
        );
    ...
    default:
      ...
      if (!TubeBody) {
        ...
        console.log(
          colors29.bold.green("@info: ") + `preparing payload for ${TubeBody.title}`
        );
...
async function AudioLowest(input) {
  try {
    ...
    if (error instanceof ZodError) {
      return {
        message: colors29.red("@error: ") + error.errors.map((error2) => error2.message).join(", "),
        status: 500
      };
    } else if (error instanceof Error) {
      return {
        message: colors29.red("@error: ") + error.message,
        status: 500
      };
    } else {
      return {
        message: colors29.red("@error: ") + "internal server error",
        status: 500
      };
    }
  }
