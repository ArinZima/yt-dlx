...
import path21__default from 'path';
...
var getDirname = () => path21__default.dirname(getFilename());
...
async function SearchVideos(input) {
  ...
  let playlistMeta = [];
  let TubeResp;
  console.log(colors29.green("@scrape:"), "booting chromium...");
  switch (input.type) {
    case "video":
      ...
      if (screenshot) {
        ...
        console.log(colors29.yellow("@scrape:"), "took snapshot...");
      }
      ...
      console.log(
        colors29.green("@info:"),
        colors29.white("scrapping done for"),
        query
      );
      TubeResp = metaTube;
      break;
    ...
    default:
      console.log(
        colors29.red("@error:"),
        colors29.white("wrong filter type provided.")
      );
      TubeResp = void 0;
      break;
  }
...
function help() {
  console.log(
    ...
    colors29.green("yt-dlx."),
    "Consider",
    colors29.green("\u{1F31F}starring"),
    "the github repo",
    colors29.green("https://github.com/yt-dlx\n")
  );
...
async function Engine({
  ...
  while (maxT > 0) {
    const enginePath = path21.join(dirC, "util", "engine");
    if (fs2.existsSync(enginePath)) {
      pLoc = enginePath;
      break;
    } else {
      dirC = path21.join(dirC, "..");
      maxT--;
    }
  }
  ...
  if (!videoId) {
    ...
    if (!TubeBody[0]) {
      ...
      console.log(
        ...
        colors29.green(TubeBody[0].title)
      );
    ...
    if (query === void 0) {
      ...
      if (resp === void 0) {
        ...
        console.log(
          ...
          colors29.green(resp.playlistTitle),
...
async function AudioLowest(input) {
  ...
  if (engineData === void 0) {
    throw new Error(
      colors29.red("@error: ") + "unable to get response from youtube."
    );
  } else {
    ...
    const folder = output ? path21.join(process.cwd(), output) : process.cwd();
    ...
    if (stream) {
      return {
        ffmpeg: ffmpeg2,
        filename: output ? path21.join(folder, filename) : filename.replace("_)_", ")_")
      };
    } else {
      await new Promise((resolve, _reject) => {
        ffmpeg2.output(path21.join(folder, filename.replace("_)_", ")_")));
...
async function AudioQualityCustom(input) {
  ...
  if (engineData === void 0) {
    ...
    const customData = engineData.HighAudioDRC.filter((op) => op.format_note === quality) || engineData.HighAudio.filter((op) => op.format_note === quality) || engineData.LowAudioDRC.filter((op) => op.format_note === quality) || engineData.LowAudio.filter((op) => op.format_note === quality);
    if (!customData) {
      throw new Error(
        colors29.red("@error: ") + quality + " not found in the video."
      );
    }
    const title = engineData.metaData.title.replace(
      /[^a-zA-Z0-9_]+/g,
...
async function ListAudioLowest(input) {
  ...
  for (const pURL of query) {
    try {
      ...
      console.log(colors29.red("@error:"), error);
      continue;
    }
  }
  console.log(
    ...
    colors29.yellow(vDATA.size.toString())
  );
...
// core/pipes/video/single/VideoLowest.ts
var qconf7 = z.object({
  query: z.string().min(1),
  output: z.string().optional(),
  stream: z.boolean().optional(),
...
async function VideoLowest(input) {
  ...
  if (engineData === void 0) {
    ...
    ffmpeg2.addInput(engineData.metaData.thumbnail);
...
// core/pipes/video/single/VideoHighest.ts
var qconf8 = z.object({
  query: z.string().min(1),
  output: z.string().optional(),
  stream: z.boolean().optional(),
...
var program = async () => {
  ...
  switch (command) {
    case "install:deps":
      while (maxTries > 0) {
        const enginePath = path21.join(currentDir, "util");
        if (fs2.existsSync(enginePath)) {
          uLoc = enginePath;
          break;
        } else {
          currentDir = path21.join(currentDir, "..");
          maxTries--;
        }
      }
      const rox = spawn("sh", [
      ...
      await Promise.all([
        new Promise((resolve, reject2) => {
          rox.stdout.on("data", (stdout) => {
            console.log(colors29.green("@stdout:"), stdout.toString().trim());
          });
          rox.on("close", (code) => {
            if (code === 0)
              resolve();
        ...
        new Promise((resolve, reject2) => {
          rox.stderr.on("data", (stderr) => {
            console.log(colors29.yellow("@stderr:"), stderr.toString().trim());
          });
          rox.on("close", (code) => {
            if (code === 0)
              resolve();
