  ...
  try {
    ...
    switch (true) {
      case error instanceof ZodError:
        throw new Error(
          colors28.red("@error: ") + error.errors.map((error2) => error2.message).join(", ")
        );
      case error instanceof Error:
        throw new Error(colors28.red("@error: ") + error.message);
      default:
        throw new Error(colors28.red("@error: ") + "internal server error");
    }
  }
}
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      child.on("close", async (code) => {
        if (code !== 0 && !ipAddress)
          throw new Error("internal server error");
        console.log(colors28.green("@info:"), "new socks5 ip", ipAddress);
...
async function proTube({
  ...
  if (ipAddress) {
    console.log(colors28.green("@ffmpeg:"), "using proxy ip", ipAddress);
