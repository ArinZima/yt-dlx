      ...
      case "video":
        TubeResp = await retry(async () => {
          ...
          spinnies.succeed(spin, {
            text: colors23.green("@info: ") + colors23.white("scrapping done")
          });
          if (page)
            await page.close();
          if (browser)
  ...
  try {
    ...
    switch (true) {
      case (!query || query.trim() === ""):
        throw new Error(colors23.red("@error: ") + "'query' is required.");
      case (/https/i.test(query) && /list/i.test(query)):
        throw new Error(
          colors23.red("@error: ") + "use extract_playlist_videos()."
        );
      case (/https/i.test(query) && !/list/i.test(query)):
    ...
    switch (videoId) {
      case void 0:
        ...
        if (!TubeBody || TubeBody.length === 0) {
          throw new Error(
            colors23.red("@error: ") + "no data returned from server."
          );
        } else if (TubeBody[0]) {
          console.log(
            colors23.green("@info: ") + `preparing payload for ${TubeBody[0].title}`
          );
      ...
      default:
        ...
        console.log(
          colors23.green("@info: ") + `preparing payload for ${TubeBody.title}`
        );
        respEngine = await Engine(TubeBody.videoLink);
        break;
    }
    if (respEngine === void 0) {
      throw new Error(colors23.red("@error: ") + "no data returned from server.");
    } else
      return respEngine;
  } catch (error) {
    if (error instanceof Error) {
    ...
    for (const videoLink of playlistUrls) {
      ...
      for (let i = 0; i < resp.playlistVideos.length; i++) {
        try {
          ...
          console.error(colors23.bold.red("@error: "), error);
        }
      }
    }
    return proTubeArr;
