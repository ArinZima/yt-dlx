...
async function SearchVideos(input) {
  ...
  let playlistMeta = [];
  let TubeResp;
  console.log(colors23.green("@scrape:"), "booting chromium...");
  switch (input.type) {
    case "video":
      ...
      if (screenshot) {
        ...
        console.log(colors23.yellow("@scrape:"), "took snapshot...");
      }
      ...
      console.log(
        colors23.green("@info:"),
        colors23.white("scrapping done for"),
        query
      );
      TubeResp = metaTube;
      break;
    ...
    default:
      console.log(
        colors23.red("@error:"),
        colors23.white("wrong filter type provided.")
      );
      TubeResp = void 0;
      break;
  }
...
function help() {
  console.log(
    ...
    colors23.green("yt-dlx."),
    "Consider",
    colors23.green("\u{1F31F}starring"),
    "the github repo",
    colors23.green("https://github.com/yt-dlx\n")
  );
...
async function Engine({
  ...
  while (maxT > 0) {
    ...
    switch (true) {
      case fs2.existsSync(enginePath):
        pLoc = enginePath;
        maxT = 0;
        break;
      default:
  ...
  if (!videoId) {
    ...
    if (!TubeBody[0]) {
      ...
      console.log(
        ...
        colors23.green(TubeBody[0].title)
      );
    ...
    if (query === void 0) {
      ...
      if (resp === void 0) {
        ...
        console.log(
          ...
          colors23.green(resp.playlistTitle),
...
function progressBar(prog) {
  ...
  let color = colors23.green;
  if (prog.percent > 98)
    prog.percent = 100;
  if (prog.percent < 25)
    color = colors23.red;
  else if (prog.percent < 50)
    color = colors23.yellow;
...
async function AudioLowest(input) {
  ...
  if (engineData === void 0) {
    throw new Error(
      colors23.red("@error: ") + "unable to get response from YouTube."
    );
  } else {
    ...
    if (adata) {
      ...
      throw new Error(colors23.red("@error: ") + "No audio data found.");
    ...
    if (stream) {
      ...
      await new Promise((resolve, reject2) => {
        ...
        ff.on("end", () => resolve());
        ff.on("error", (error) => {
          reject2(new Error(colors23.red("@error: ") + error.message));
        });
        ff.run();
      });
    }
...
async function ListAudioLowest(input) {
  ...
  for (const pURL of query) {
    try {
      ...
      console.log(colors23.red("@error:"), error);
      continue;
    }
  }
  console.log(
    ...
    colors23.yellow(vDATA.size.toString())
  );
  for (const video of vDATA) {
    try {
      ...
      await new Promise((resolve, _reject) => {
        ffmpeg6.output(path15.join(folder, filename.replace("_)_", ")_")));
        ffmpeg6.on("end", () => resolve());
        ffmpeg6.on("error", (error) => {
          throw new Error(colors23.red("@error: ") + error.message);
        });
        ffmpeg6.run();
      });
    } catch (error) {
...
async function VideoLowest(input) {
  ...
  if (engineData === void 0) {
    ...
    if (vdata) {
      ...
      throw new Error(colors23.red("@error: ") + "No video data found.");
...
var program = async () => {
  ...
  switch (command) {
      ...
      await Promise.all([
        new Promise((resolve, reject2) => {
          rox.stdout.on("data", (stdout) => {
            console.log(colors23.green("@stdout:"), stdout.toString().trim());
          });
          rox.on("close", (code) => {
            if (code === 0)
              resolve();
        ...
        new Promise((resolve, reject2) => {
          rox.stderr.on("data", (stderr) => {
            console.log(colors23.yellow("@stderr:"), stderr.toString().trim());
          });
          rox.on("close", (code) => {
            if (code === 0)
              resolve();
    ...
    case "version":
    case "v":
      console.error(colors23.green("Installed Version: yt-dlx@" + version));
      break;
    case "help":
    case "h":
      core_default().info().help().then((data) => {
        ...
        console.error(colors23.red(error));
        process.exit();
      });
      break;
    case "extract":
    case "e":
      if (!proTube2 || !proTube2.query || proTube2.query.length === 0) {
        console.error(colors23.red("error: no query"));
      } else
    ...
    case "aqc":
      ...
      if (!proTube2 || !proTube2.format || proTube2.format.length === 0) {
        console.error(colors23.red("error: no format"));
      }
