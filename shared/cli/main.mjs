...
async function Engine(query) {
  try {
    ...
    while (maxTries2 > 0) {
      const enginePath = path3.join(currentDir2, "util", "engine");
      if (fs.existsSync(enginePath)) {
        proLoc = enginePath;
        break;
      } else {
        currentDir2 = path3.join(currentDir2, "..");
        maxTries2--;
      }
    }
    if (proLoc !== "") {
...
function gpuffmpeg(input, verbose) {
  const ffmpeg = fluent().input(input).on("start", (command) => {
    if (verbose)
      console.log(colors24.green("\n@ffmpeg:"), command);
  }).on("progress", ({ percent, timemark }) => {
    progressBar_default({ timemark, percent });
  }).on("end", () => console.log(colors24.green("\n@ffmpeg:"), "ended")).on("close", () => console.log(colors24.green("\n@ffmpeg:"), "closed")).on("finish", () => console.log(colors24.green("\n@ffmpeg:"), "finish")).on("error", (e) => console.error(colors24.red("\n@ffmpeg:"), e.message));
  while (maxTries > 0) {
  ...
  switch (true) {
    case (vendor && vendor.includes("NVIDIA")):
      console.log(colors24.green("@ffmpeg: using GPU " + vendor));
