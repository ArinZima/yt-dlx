...
async function closers(browser2) {
  try {
    ...
    console.error(colors33.red("@error:"), error);
  }
}
  ...
  try {
    ...
    switch (true) {
      case error instanceof Error:
        throw new Error(colors33.red("@error: ") + error.message);
      default:
        throw new Error(colors33.red("@error: ") + "internal server error");
    }
  }
}
...
async function SearchVideos(input) {
  try {
    ...
    let snapshot;
    spinnies.add(spin, {
      text: colors33.green("@scrape: ") + "booting chromium..."
    });
    switch (input.type) {
      case "video":
        TubeResp = await retry(async () => {
          ...
          spinnies.update(spin, {
            text: colors33.yellow("@scrape: ") + "waiting for hydration..."
          });
          if (screenshot) {
            ...
            spinnies.update(spin, {
              text: colors33.yellow("@scrape: ") + "took snapshot..."
            });
          }
          ...
          spinnies.succeed(spin, {
            text: colors33.green("@info: ") + colors33.white("scrapping done for ") + query
          });
          return metaTube;
        }, retryOptions);
      ...
      default:
        spinnies.fail(spin, {
          text: colors33.red("@error: ") + colors33.white("wrong filter type provided.")
        });
        await closers(browser);
        return void 0;
    }
    ...
    switch (true) {
      case error instanceof ZodError:
        throw new Error(
          colors33.red("@error: ") + error.errors.map((error2) => error2.message).join(", ")
        );
      case error instanceof Error:
        throw new Error(colors33.red("@error: ") + error.message);
      default:
...
async function Engine({
  ...
  try {
    ...
    if (proLoc !== "") {
      ...
      throw new Error(
        colors33.red("@error: ") + "could not find the engine file."
      );
    }
...
async function Agent({
  ...
  try {
    ...
    console.log(
      colors33.green("@info:"),
      "using",
      colors33.green("yt-dlx"),
      "version",
      colors33.green(version)
    );
    if (!videoId) {
      ...
      if (!TubeBody[0]) {
        throw new Error(
          colors33.red("@error: ") + "Unable to get response from YouTube..."
        );
      } else {
        console.log(
          colors33.green("@info:"),
          `preparing payload for`,
          colors33.green(TubeBody[0].title)
        );
      ...
      if (query === void 0) {
        ...
        if (resp === void 0) {
          ...
          console.log(
            ...
            colors33.green(resp.playlistTitle),
...
var progressBar = (prog) => {
  ...
  let color = colors33.green;
  if (prog.percent > 98)
    prog.percent = 100;
  if (prog.percent < 25)
    color = colors33.red;
  else if (prog.percent < 50)
    color = colors33.yellow;
...
function gpuffmpeg({
  ...
  const ffmpeg = fluent(input).on("start", (command) => {
    if (verbose)
      console.log(colors33.green("@ffmpeg:"), command);
  }).on("progress", (prog) => progressBar(prog)).on("end", () => console.log("\n")).on("error", (e) => console.error(colors33.red("\n@ffmpeg:"), e.message));
  while (maxTries > 0) {
  ...
  switch (true) {
    case (vendor && vendor.includes("NVIDIA")):
      console.log(colors33.green("@ffmpeg:"), "using GPU", colors33.green(vendor));
      ffmpeg.withInputOption("-hwaccel cuda");
      ffmpeg.withVideoCodec("h264_nvenc");
      break;
    default:
      console.log(
        colors33.yellow("@ffmpeg:"),
...
async function lowEntry(metaBody) {
  if (!metaBody || metaBody.length === 0) {
    console.log(colors33.red("@error:"), "sorry no downloadable data found");
    return void 0;
  }
    ...
    if (engineData === void 0) {
      ...
      if (sortedData === void 0) {
        ...
        console.log(
          ...
          colors33.green("\u{1F31F}starring"),
          "the github repo",
          colors33.green("https://github.com/yt-dlx")
        );
      }
    }
  } catch (error) {
...
async function AudioQualityCustom(input) {
  try {
    ...
    if (engineData === void 0) {
      ...
      if (!customData) {
        throw new Error(
          colors33.red("@error: ") + quality + " not found in the video."
        );
      }
      const title = engineData.metaTube.title.replace(
        /[^a-zA-Z0-9_]+/g,
...
async function AudioVideoQualityCustom(input) {
  try {
    ...
    if (engineData === void 0) {
      ...
      if (AudioData === void 0) {
        throw new Error(
          colors33.red("@error: ") + AQuality + " not found in the video."
        );
      } else if (VideoData === void 0) {
        throw new Error(
          colors33.red("@error: ") + VQuality + " not found in the video."
        );
      } else {
        const ffmpeg = ffmpeg_default({
          input: VideoData.AVDownload.mediaurl,
...
var program = async () => {
  ...
  switch (command) {
    case "version":
    case "v":
      console.error(colors33.green("Installed Version: yt-dlx@" + version));
      break;
    case "help":
    case "h":
      core_default().info().help().then((data) => {
        ...
        console.error(colors33.red(error));
        process.exit();
      });
      break;
    case "extract":
    case "e":
      if (!proTube || !proTube.query || proTube.query.length === 0) {
        console.error(colors33.red("error: no query"));
      } else
    ...
    case "aqc":
      ...
      if (!proTube || !proTube.format || proTube.format.length === 0) {
        console.error(colors33.red("error: no format"));
      }
