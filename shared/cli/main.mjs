...
var getDirname = () => path9__default.dirname(getFilename());
...
async function VideoInfo(input) {
  ...
  if (screenshot) {
    ...
    console.log(colors15.yellow("@scrape:"), "took snapshot...");
  }
  ...
  console.log(
    colors15.green("@info:"),
    colors15.white("scrapping done for"),
    colors15.green(query)
  );
...
async function SearchVideos(input) {
  ...
  switch (input.type) {
    ...
    default:
      console.log(
        colors15.red("@error:"),
        colors15.white("wrong filter type provided.")
      );
      TubeResp = void 0;
      break;
  }
  ...
  if (!videoId) {
    ...
    if (!TubeBody[0])
      ...
      console.log(
        ...
        colors15.green(TubeBody[0].title)
      );
    ...
    if (query === void 0) {
      ...
      if (!resp) {
        ...
        console.log(
          ...
          colors15.green(resp.playlistTitle),
...
async function AudioLowest(input) {
  ...
  if (engineData === void 0) {
    throw new Error(
      colors15.red("@error: ") + "unable to get response from YouTube."
    );
  } else {
    ...
    const folder = output ? path9.join(process.cwd(), output) : process.cwd();
    ...
    if (stream) {
      return {
        ffmpeg: ff,
        filename: output ? path9.join(folder, filename) : filename.replace("_)_", ")_")
      };
    } else {
      await new Promise((resolve, reject2) => {
        ff.output(path9.join(folder, filename.replace("_)_", ")_")));
        ff.on("end", () => resolve());
        ff.on("error", (error) => {
...
var program = async () => {
  ...
  switch (command) {
    case "install:deps":
      while (maxTries > 0) {
        const enginePath = path9.join(currentDir, "util");
        if (fs2.existsSync(enginePath)) {
          uLoc = enginePath;
          break;
        } else {
          currentDir = path9.join(currentDir, "..");
          maxTries--;
        }
      }
      const rox = spawn("sh", [
      ...
      await Promise.all([
        new Promise((resolve, reject2) => {
          rox.stdout.on("data", (stdout) => {
            console.log(colors15.green("@stdout:"), stdout.toString().trim());
          });
          rox.on("close", (code) => {
            if (code === 0)
              resolve();
        ...
        new Promise((resolve, reject2) => {
          rox.stderr.on("data", (stderr) => {
            console.log(colors15.yellow("@stderr:"), stderr.toString().trim());
          });
          rox.on("close", (code) => {
            if (code === 0)
              resolve();
    ...
    case "version":
    case "v":
      console.error(colors15.green("Installed Version: yt-dlx@" + version));
      break;
    case "help":
    case "h":
    ...
    case "e":
      if (!proTube || !proTube.query || proTube.query.length === 0) {
        console.error(colors15.red("error: no query"));
      } else
        await core_default.info.extract({
          ...
          console.error(colors15.red(error));
          process.exit();
        });
      break;
    case "audio-highest":
    ...
    case "avh":
      if (!proTube || !proTube.query || proTube.query.length === 0) {
        ...
        await core_default.AudioVideo.Single.Highest({
          query: proTube.query
        }).then((data2) => {
          console.log(data2);
          process.exit();
    ...
    case "avl":
      if (!proTube || !proTube.query || proTube.query.length === 0) {
        ...
        await core_default.AudioVideo.Single.Lowest({
          query: proTube.query
        }).then((data2) => {
          console.log(data2);
          process.exit();
