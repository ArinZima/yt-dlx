...
var browser;
var page;
async function crawler(verbose, _torprox) {
  try {
...
async function runFunc(runLogic) {
  let retry4 = 0;
  while (retry4 < 6) {
    try {
      ...
      retry4++;
      console.log(colors29.yellow("@retry:"), `${retry4}/6.`);
      if (retry4 >= 6)
        throw new Error(colors29.red("@error:"), error.message);
      const timeout = Math.min(6e3, Math.max(1e3, retry4 * 1e3));
...
async function gpuffmpeg({
  ...
  const response = await runFunc(async () => {
    ...
    const ffmpeg = fluent(input).on("start", (command) => {
      if (verbose)
        console.log(colors29.green("@ffmpeg:"), command);
    }).on("progress", (prog) => progressBar(prog, size)).on("end", () => console.log("\n")).on("error", (e) => console.error(colors29.red("\n@ffmpeg:"), e.message));
    while (maxTries > 0) {
    ...
    const response = await runFunc(
      async () => {
        ...
        if (engineData === void 0) {
          ...
          const ffmpeg = await ffmpeg_default({
