...
async function AudioLowest(input) {
    try {
        const { query, output, stream, verbose, filter } = await qconf$c.parseAsync(input);
...
async function AudioHighest(input) {
    try {
        const { query, output, stream, verbose, filter } = await qconf$b.parseAsync(input);
        const engineData = await Agent({ query, verbose });
        if (engineData === undefined) {
            throw new Error(colors.red("@error: ") + "unable to get response from youtube.");
        }
...
async function AudioQualityCustom(input) {
    try {
        const { query, stream, verbose, output, quality, filter } = await qconf$a.parseAsync(input);
        const engineData = await Agent({ query, verbose });
        if (engineData === undefined) {
            throw new Error(colors.red("@error: ") + "unable to get response from youtube.");
        }
...
async function ListAudioLowest(input) {
    try {
        const { query, output, verbose, filter } = await qconf$9.parseAsync(input);
...
async function ListAudioHighest(input) {
    try {
        const { query, output, verbose, filter } = await qconf$8.parseAsync(input);
        const playlistData = await web.search.PlaylistInfo({ query });
        if (playlistData === undefined) {
            throw new Error(colors.red("@error: ") + "unable to get response from youtube.");
        }
...
async function VideoLowest(input) {
    try {
        const { query, stream, verbose, output, filter } = await qconf$7.parseAsync(input);
        const engineData = await Agent({ query, verbose });
        if (engineData === undefined) {
            throw new Error(colors.red("@error: ") + "unable to get response from youtube.");
        }
...
async function VideoQualityCustom(input) {
    try {
        const { query, stream, verbose, output, quality, filter } = await qconf$5.parseAsync(input);
        const engineData = await Agent({ query, verbose });
        if (engineData === undefined) {
            throw new Error(colors.red("@error: ") + "unable to get response from youtube.");
        }
...
async function ListVideoLowest(input) {
    try {
        const { query, output, verbose, filter } = await qconf$4.parseAsync(input);
        const playlistData = await web.search.PlaylistInfo({ query });
        if (playlistData === undefined) {
            throw new Error(colors.red("@error: ") + "unable to get response from youtube.");
        }
...
async function ListVideoHighest(input) {
    try {
        const { query, verbose, output, filter } = await qconf$3.parseAsync(input);
        const playlistData = await web.search.PlaylistInfo({ query });
        if (playlistData === undefined) {
            throw new Error(colors.red("@error: ") + "unable to get response from youtube.");
        }
...
async function AudioVideoHighest(input) {
    try {
        const { query, stream, verbose, output, filter } = await qconf$1.parseAsync(input);
        const engineData = await Agent({ query, verbose });
        if (engineData === undefined) {
            throw new Error(colors.red("@error: ") + "unable to get response from youtube.");
        }
...
const qconf = z$1.object({
    query: z$1.string().min(1),
    output: z$1.string().optional(),
    stream: z$1.boolean().optional(),
    verbose: z$1.boolean().optional(),
    AQuality: z$1.enum(["high", "medium", "low", "ultralow"]),
    VQuality: z$1.enum([
        "144p",
        "240p",
        "360p",
...
async function AudioVideoQualityCustom(input) {
    try {
        const { query, stream, verbose, output, VQuality, AQuality, filter } = await qconf.parseAsync(input);
        const engineData = await Agent({ query, verbose });
        if (engineData === undefined) {
            throw new Error(colors.red("@error: ") + "unable to get response from youtube.");
        }
