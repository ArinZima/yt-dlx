...
async function Engine({
  ...
  console.log(
    colors14__default.default.bold.green("INFO: ") + `\u2B55 using yt-core version <(${version})>` + colors14__default.default.reset("")
  );
  if (!query || query.trim() === "") {
    console.log(
      colors14__default.default.bold.red("ERROR: ") + "\u2757 'query' is required..." + colors14__default.default.reset("")
    );
    return;
  }
  if (/https/i.test(query) && /list/i.test(query)) {
    console.log(
      colors14__default.default.bold.red("ERROR: ") + "\u2757 use extract_playlist_videos() for playlists..." + colors14__default.default.reset("")
    );
    return;
  } else if (/https/i.test(query) && !/list/i.test(query)) {
    console.log(
      colors14__default.default.bold.green("INFO: ") + `\u2B55 fetching metadata for: <(${query})>` + colors14__default.default.reset("")
    );
  ...
  switch (videoId) {
    case null:
      ...
      if (TubeBody === null) {
        console.log(
          colors14__default.default.bold.red("ERROR: ") + "\u2757 no data returned from server..." + colors14__default.default.reset("")
        );
        return;
      }
      TubeBody = JSON.parse(TubeBody);
      console.log(
        colors14__default.default.bold.green("INFO: ") + `\u{1F4E1} preparing payload for <(${TubeBody[0].Title} Author: ${TubeBody[0].Uploader})>` + colors14__default.default.reset("")
      );
    ...
    default:
      ...
      TubeBody = JSON.parse(TubeBody);
      console.log(
        colors14__default.default.bold.green("INFO: ") + `\u{1F4E1} preparing payload for <(${TubeBody.Title} Author: ${TubeBody.Uploader})>` + colors14__default.default.reset("")
      );
  ...
  switch (TubeCore) {
    ...
    default:
      console.log(
        colors14__default.default.bold.green("INFO: ") + "\u2763\uFE0F Thank you for using yt-core! If you enjoy the project, consider\nstarring the GitHub repo: https://github.com/shovitdutta/mixly/yt-core" + colors14__default.default.reset("")
      );
...
var progressBar = (percent, _metaSpin) => {
  ...
  let color = colors14__default.default.green;
  if (percent < 20)
    color = colors14__default.default.red;
  else if (percent < 80)
    color = colors14__default.default.yellow;
