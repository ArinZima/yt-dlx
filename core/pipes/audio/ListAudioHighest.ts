...
export default async function ListAudioHighest(
  ...
  try {
    ...
    for (const url of playlistUrls) {
      ...
      const newList = await JSON.parse(metaList);
      for (const meta of newList.Videos) {
        let TubeBody = await scrape(meta);
        if (TubeBody === null) {
          return {
            message: "Unable to get response from YouTube...",
            status: 500,
        ...
        const newTube = await JSON.parse(TubeBody)[0];
        const metaBody = await ytCore({ query: newTube.Link });
