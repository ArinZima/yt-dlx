...
export default function ListAudioCustom(_a) {
    return __awaiter(this, arguments, void 0, function* ({ query, output, filter, verbose, onionTor, resolution, }) {
        try {
            ZodSchema.parse({
                query,
                output,
            ...
            for (const purl of query) {
                try {
                    const playlistId = yield YouTubeID(purl);
                    if (!playlistId) {
                        console.log(colors.red("@error: "), "@error: invalid playlist", purl);
                        continue;
                    }
                    else {
                        const punique = yield web.browserLess.playlistVideos({
                            playlistId,
                        });
