...
async function ytxc(query) {
  let pushTube = [];
  let proLoc = path.join(__dirname, "backend", "util", "Engine");
...
async function Engine({
  ...
  console.log(
    colors17__default.default.bold.green("@info: ") + `using yt-dlx version <(${version})>` + colors17__default.default.reset("")
  );
  if (!query || query.trim() === "") {
    console.log(
...
var program = async () => {
  ...
  switch (command) {
    case "version":
    case "v":
      console.error(colors17__default.default.green("Installed Version: yt-dlx@" + version));
      break;
    case "help":
    case "h":
